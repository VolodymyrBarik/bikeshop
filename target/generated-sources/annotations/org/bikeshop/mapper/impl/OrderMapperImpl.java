...
public class OrderMapperImpl implements OrderMapper {
    ...
    public OrderResponseDto toDto(Order order) {
        ...
        Set<OrderItemResponseDto> set = orderItemListToOrderItemResponseDtoSet( order.getOrderItems() );
        if ( set != null ) {
    ...
    protected Set<OrderItemResponseDto> orderItemListToOrderItemResponseDtoSet(List<OrderItem> list) {
        if ( list == null ) {
            return null;
        }

        Set<OrderItemResponseDto> set = new LinkedHashSet<OrderItemResponseDto>( Math.max( (int) ( list.size() / .75f ) + 1, 16 ) );
        for ( OrderItem orderItem : list ) {
            set.add( orderItemMapper.toResponseDto( orderItem ) );
        }
